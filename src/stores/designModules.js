...
import { getDesignModules, getArchivedDesignModules, checkTauriAvailable, } from '../utils/tauriCommands';
...
export const useDesignModulesStore = create((set, get) => ({
    modules: [],
    loading: false,
    error: null,
    tauriAvailable: null,
    ...
    updateLocalStatuses: (ids, status) => set((state) => ({
        modules: state.modules.map((m) => ids.includes(m.id) ? { ...m, status } : m),
    })),
    removeLocal: (ids) => set((state) => ({ modules: state.modules.filter((m) => !ids.includes(m.id)) })),
...
// Helper selectors
export function selectFilteredSorted(state) {
...
export function selectPaged(state, list) {
